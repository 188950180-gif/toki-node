...
use toki_core::{Address, Block, Hash};
...
pub enum UpgradeStatus {
    ...
    Pending,
    /// 投票中
    Voting { start_height: u64, end_height: u64 },
    /// 已通过
    Passed { vote_height: u64 },
    /// 锁定期
    Locked { lock_start: u64, lock_end: u64 },
    /// 待执行
    ReadyToExecute { execute_height: u64 },
    /// 执行中
    Executing,
    /// 已完成
    Completed { execute_height: u64 },
    /// 已取消
...
pub enum UpgradeKind {
    ...
    /// 治理规则升级
    Governance { new_rules: Vec<u8> },
    /// 经济模型升级
    Economic { new_params: Vec<u8> },
    /// 客户端升级
    Client {
        new_version: String,
        binary_url: String,
...
impl AutoUpgradeManager {
    ...
    pub fn propose(
        ...
        let id =
            Hash::from_data(&format!("{:?}{:?}{}", kind, proposer, self.current_height).as_bytes());
