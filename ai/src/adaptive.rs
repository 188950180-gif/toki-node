...
impl AdaptiveController {
    ...
    pub fn adjust(&mut self, metrics: &NetworkMetrics) -> AdaptiveParams {
        ...
        if load_factor > 0.8 {
            ...
            self.current_params.base_tx_fee = 
                (self.current_params.base_tx_fee as f64 * 1.1) as u64;
        ...
        if metrics.peer_count < 5 {
            ...
            info!("连接少，增加最大连接数至 {}", self.current_params.max_connections);
        }
        ...
        if metrics.memory_usage > 0.8 {
            ...
            info!("内存紧张，减小交易池至 {}", self.current_params.max_tx_pool_size);
        }
    ...
    pub fn analyze_trend(&self) -> SystemTrend {
        ...
        let recent: Vec<_> = self.history_window.iter()
            .rev()
            .take(10)
            .collect();
        
        let older: Vec<_> = self.history_window.iter()
            .rev()
            .skip(10)
            .take(10)
            .collect();
        ...
        let recent_load: f64 = recent.iter()
            .map(|m| if m.capacity > 0 { m.tx_count as f64 / m.capacity as f64 } else { 0.0 })
            .sum::<f64>() / recent.len() as f64;
        
        let older_load: f64 = older.iter()
            .map(|m| if m.capacity > 0 { m.tx_count as f64 / m.capacity as f64 } else { 0.0 })
